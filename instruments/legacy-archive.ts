...
/**
 * Archivo histórico de instrumentos.
 *
 * Estas definiciones pertenecen a la implementación manual anterior al motor de
 * incorporación de instrumentos. Se conservan por una sola razón: un expediente
 * que ya las aplicó tiene que poder seguir leyéndose —sus resultados, su informe
 * y su evidencia dependen de la estructura de subtests y baremos de aquí—.
 *
 * No son catálogo. `lifecycle: 'ARCHIVED'` las deja fuera de la biblioteca, de
 * la búsqueda y del motor de recomendación, y ninguna evaluación nueva puede
 * llegar a ellas. Sólo `getInstrument` las resuelve, y sólo para leer historia.
 */
...
export const archivedInstruments: Instrument[] = [
  {
    id: 'test-abc',
    version: '1.0',
    nombre: 'TEST ABC',
